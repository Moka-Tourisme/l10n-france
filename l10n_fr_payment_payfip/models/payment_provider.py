...
class PayFIPProvider(models.Model):
    ...
    def _get_compatible_providers(self, company_id, partner_id, amount, currency_id=None, **kwargs):
        """ Override of `payment` to also expose providers in the custom 'activation' state.

        The base method only matches providers in the 'enabled' or 'test' states. PayFIP adds an
        'activation' state (real PayFIP contract, but in activation/qualification mode), which must
        remain payable. We let `super()` handle the standard providers, then add back the compatible
        'activation' providers applying the same publication, country and currency rules.
        """
        providers = super()._get_compatible_providers(
            company_id, partner_id, amount, currency_id=currency_id, **kwargs
        )

        activation_providers = self.env['payment.provider'].search([
        ...
        if not self.env.user._is_internal():
            activation_providers = activation_providers.filtered('is_published')

        # Handle the partner country; allow all countries if the list is empty.
        partner = self.env['res.partner'].browse(partner_id)
        if partner.country_id:
            activation_providers = activation_providers.filtered(
                lambda p: (
        ...
        return providers + activation_providers
